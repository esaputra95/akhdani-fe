...
const FilterReport = ({ dataWarehouse, filter, handleOnChangeSelect, handleOnFilter, dataSupplier, dataUser }) => {
    return (
        <>
            ...
            <Grid style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'space-between', marginTop: 2 }}>
                ...
                <Grid style={{ marginTop: 12 }}>
                    <Autocomplete
                        ...
                        onChange={(e, val) => handleOnFilter('supplier', val)}
                        sx={{ width: 260 }}
                        value={filter?.supplier?.label ?? ''}
                        renderInput={(params) => (
                            <TextField onChange={(val) => handleOnChangeSelect('supplier', val)} {...params} name="name" label="Supplier" />
                        )}
                    />
                </Grid>
                <Grid style={{ marginTop: 12 }}>
                ...
                <Grid style={{ marginTop: 12 }}>
                    <Autocomplete
                        ...
                        options={[
                            ...
                            { label: 'Belum diterima / Belum semua diterima', value: 'not-received' }
                        ]}
                        sx={{ width: 260 }}
                        onChange={(e, val) => handleOnFilter('received', val)}
                        renderInput={(params) => <TextField {...params} label="Status Terima" />}
                    />
                </Grid>
                <Grid style={{ marginTop: 12 }}>
                    <Autocomplete
