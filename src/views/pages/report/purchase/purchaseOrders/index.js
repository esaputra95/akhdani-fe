...
import { storeReportPurchaseOrder, storeSupplierSelect, storeWarehouseSelect, storeUserSelect } from 'services/endPoint';
...
const ReportPurchaseOrders = () => {
    ...
    const { useGet: getPurchaseOrder, data: dataPurchaseOrder } = useFetch();
    ...
    const getDataMaster = useCallback(
        ...
        [queryFormWarehouse, queryFormSupplier, queryFormUser]
    );
    ...
    const handleOpen = async () => {
        ...
        await getPurchaseOrder(storeReportPurchaseOrder, { ...filter });
        setIsLoading(false);
    };
    ...
    return (
        <MainCard title="Laporan Pesanan Pembelian">
            <FilterReport
            ...
            <Dialog maxWidth="xl" open={openDialog} onClose={handleOpen} style={{ overflow: 'auto' }}>
                ...
                <DialogContent>
                    <TableReport data={dataPurchaseOrder} filter={filter} typeReport={typeReport} />
                    {isloading ? (
                        <>
