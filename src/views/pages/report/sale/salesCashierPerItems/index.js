...
import {
    storeReportSaleCashierPeritems,
    storeMemberSelect,
...
const ReportSaleCashierPerItems = () => {
    ...
    const { useGet: getSaleCashierPerItems, data: dataSaleCashierPerItems } = useFetch();
    ...
    const handleOpen = async () => {
        ...
        await getSaleCashierPerItems(storeReportSaleCashierPeritems, { ...filter, limit: 10000 });
        setIsLoading(false);
    };
    ...
    return (
        <MainCard title="Laporan Penjualan Kasir Per Item">
            <FilterReport
            ...
            <Dialog maxWidth="xl" open={openDialog} onClose={handleOpen} style={{ overflow: 'auto' }}>
                <DialogTitle sx={{ width: '100%' }} id="alert-dialog-title" style={{ fontSize: 16 }}>
                    Laporan Penjualan Kasir Peritem
                </DialogTitle>
                <DialogContent>
                    <TableReport data={dataSaleCashierPerItems} filter={filter} typeReport={typeReport} isloading={isloading} />
                    {isloading ? (
                        <>
...
export default ReportSaleCashierPerItems;
