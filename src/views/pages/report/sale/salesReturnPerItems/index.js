...
import {
    storeReportSaleReturnsPeritems,
    storeMemberSelect,
...
const ReportSaleReturnsPerItems = () => {
    ...
    const { useGet: getSaleReturnsPerItems, data: dataSaleReturnsPerItems } = useFetch();
    ...
    const handleOpen = async () => {
        ...
        await getSaleReturnsPerItems(storeReportSaleReturnsPeritems, { ...filter, limit: 10000 });
        setIsLoading(false);
    };
    ...
    return (
        <MainCard title="Laporan Retur Penjualan Per Item">
            <FilterReport
            ...
            <Dialog maxWidth="xl" open={openDialog} onClose={handleOpen} style={{ overflow: 'auto' }}>
                <DialogTitle sx={{ width: '100%' }} id="alert-dialog-title" style={{ fontSize: 16 }}>
                    Laporan Retur Penjualan Peritem
                </DialogTitle>
                <DialogContent>
                    <TableReport data={dataSaleReturnsPerItems} filter={filter} typeReport={typeReport} isloading={isloading} />
                    {isloading ? (
                        <>
...
export default ReportSaleReturnsPerItems;
