...
import { storeReportSalePeritems, storeMemberSelect, storeWarehouseSelect, storeUserSelect, storeItemSelect } from 'services/endPoint';
...
const ReportSalePerItems = () => {
    ...
    const { useGet: getSalePerItems, data: dataSalePerItems } = useFetch();
    ...
    const getDataMaster = useCallback(
        ...
        [queryFormWarehouse, queryFormMember, queryFormUser, queryFormItem]
    );
    ...
    const handleOpen = async () => {
        ...
        await getSalePerItems(storeReportSalePeritems, { ...filter, limit: 10000 });
        setIsLoading(false);
    };
    ...
    return (
        <MainCard title="Laporan Penjualan Per Item">
            <FilterReport
            ...
            <Dialog maxWidth="xl" open={openDialog} onClose={handleOpen} style={{ overflow: 'auto' }}>
                <DialogTitle sx={{ width: '100%' }} id="alert-dialog-title" style={{ fontSize: 16 }}>
                    Laporan Penjualan Peritem
                </DialogTitle>
                <DialogContent>
                    <TableReport data={dataSalePerItems} filter={filter} typeReport={typeReport} isloading={isloading} />
                    {isloading ? (
                        <>
...
export default ReportSalePerItems;
